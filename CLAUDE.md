...
11. **Film Room discrepancy UX** — Flujo de resolución de conflictos poco claro para coaches nuevos. **Ampliado 2026-09-15, no resuelto — ver spec sección 46 antes de tocar esto**: al investigarlo apareció algo más gordo que falta de copy — rastreado paso a paso, ningún override/pick de alternativa de ningún entrenador parece llegar nunca al informe publicado (`mergeAndClearScoutVersions` borra `report_overrides` sin leerlo ni fusionarlo en `players.inputs` antes). Puede ser un bug real o el diseño correcto (2 lecturas posibles, detalladas en la spec) — necesita la búsqueda en Claude Desktop del diseño original de "el decantador" antes de decidir, no un fix a ciegas.
