...
1. Un pívot con señales fuertes de creación (visión élite + una señal explícita de distribución) se clasifica hoy como `interior_creadora` aunque su situación dominante sea post-up (caso real: Jokic). Es una decisión de qué información añade más valor al informe (P2), no una medición — si prefieres que el archetype describa dónde hay que defenderla en vez de cómo funciona el ataque, se invierte en 4 líneas.
2. El puente desde `motor-v2.1.ts` colapsa SG a grupo "base" siempre — un SG sin carga on-ball real (ej. Klay Thompson) sale `manejadora_secundaria` en vez de `alero_tiradora`, que es lo que realmente es. El Arquitecto propone una regla local (SG con carga on-ball real → base, si no → alero) — parche del puente legacy únicamente, no aplica al contrato nuevo donde el staff teclea el grupo directamente.

`[PENDIENTE VALIDAR CON PABLO]` ambas — con la corrección de la pregunta 2 aplicada o no, el resto del algoritmo no cambia.
