...
## 43. Barrido de pendientes "baja prioridad" de CLAUDE.md — uno descartado por premisa falsa, uno resultó ser un hallazgo de marca real (2026-09-14)

Al seguir avanzando por el propio backlog de `CLAUDE.md` (mandato "continua, y tmb continua lo q tuvieses pendiente"), dos ítems de la sección "Baja prioridad/cosmético" resultaron no ser lo que decían:

- **Ítem "ModCard SOON badge"/"Alert chips sub-text" (`md:` en `HomeMobile.tsx`) — descartado, no aplicable.** `Home.tsx` elige entre `HomeMobile.tsx`/`HomeDesktop.tsx` por JS (`useIsDesktop()`, hook con `matchMedia("(min-width: 768px)")` — el mismo breakpoint que Tailwind `md:`), no son el mismo árbol con clases responsive compartidas. `HomeMobile.tsx` nunca se monta en desktop, así que cualquier `md:` ahí sería CSS que nunca se activa. Verificado leyendo `Home.tsx` y el hook antes de tocar nada — no se hizo ningún cambio.
- **Ítem "Comentarios 'U Scout' cosméticos" — resultó ser un hallazgo de marca real, no cosmético.** Las líneas concretas que citaba el pendiente (`Home.tsx:165,169`) ya no existen (`Home.tsx` quedó reducido a un wrapper de 15 líneas en algún momento no documentado). Pero al buscar "U Scout" en todo `client/src` aparecieron **8 casos que sí son visibles a usuarios reales**, no comentarios de código:
  - Los 4 títulos de tarjeta de módulo en el Home de **móvil** (`HomeMobile.tsx`): `ucore_card_scout_title` = "U Scout" (en/es/zh), `ucore_card_schedule_title` = "U Schedule & Wellness"/"U 日程"/"U Calendario", `ucore_card_stats_title` = "U Stats" (en/es/zh) — y Playbook ni siquiera tenía clave i18n, `title="U Playbook"` fijo en el JSX, sin traducir en absoluto. El Home de **desktop** (`HomeDesktop.tsx`) ya usaba las claves correctas (`ucore_nav_*`, sin el prefijo "U ") — la marca antigua sobrevivía solo en la rama móvil.
  - 3 casos más en `ClubManagement.tsx`: la lista de módulos activables del club (`label: "U Scout"`/`"U Stats"`/`"U Playbook"`, sin i18n) y la etiqueta del selector de vista de informe por defecto del club ("U Scout default report view" en las 3 traducciones).
  - Los comentarios de código puramente internos (`App.tsx:274`, `UScoutBrand.tsx:2`, `capabilities.ts:123`, `motor-v2.1*.ts`, `stats-api.ts:359`) sí son cosméticos de verdad (nunca los ve un usuario) — esos se dejan tal cual, consistente con el criterio original del pendiente.

**Corregido reutilizando naming ya decidido, sin inventar nada nuevo**: las claves `ucore_nav_scout`/`_stats`/`_playbook` ya tenían el texto correcto (usado en el sidebar desktop) — los 4 títulos de tarjeta y las 3 etiquetas de `ClubManagement.tsx` ahora los reutilizan o replican, quitando el prefijo "U " en los 3 idiomas. Excepción deliberada: el título "Schedule & Wellness" de la tarjeta de Home (staff) y el de la lista de módulos de `ClubManagement.tsx` **conservan** el sufijo "& Wellness"/"y Bienestar"/"与健康" — no es resto de marca, es información real (el staff no tiene una tarjeta de Wellness separada, va fusionada en esta).

Verificado: `npm run check` limpio, `npx vitest run` 15/15 archivos, 224/224 pruebas. No se pudo hacer smoke test interactivo de las pantallas autenticadas (Home, ClubManagement) por no disponer de credenciales reales — verificado por lectura de código y grep de las claves i18n consumidas, no asumido.

## 44. Onboarding por módulo — tarjeta de primera vez al abrir cada módulo, más "ver tutoriales otra vez" en Ajustes (2026-09-14)

Pablo pidió extender el onboarding en 2 partes: (1) un mini-tutorial que ayude a registrarse y configurar usuario/idioma/colores, y (2) que cada módulo tenga su propia intro la primera vez que se abre. Antes de diseñar la parte 2 desde cero se preguntó por el protocolo habitual (¿ya se pensó esto en Claude Desktop?) — Pablo respondió **"que opinas que es mas comodo y funcional? tu decides"**, delegando explícitamente el criterio de diseño.

**Parte 1 — verificado que ya existía, no se rediseñó**: `OnboardingFlow.tsx` ya encadena registro → idioma → tema/colores → tutorial de 3-4 slides por rol, disparado automáticamente tras crear cuenta (`AuthGate`). No hacía falta nada nuevo aquí más allá de hacerlo repetible (ver más abajo).

**Parte 2 — decisión de diseño propia, justificada, no un rediseño de otra cosa**: en vez de otro wizard de varias slides a pantalla completa por cada módulo (fatiga real: un usuario nuevo que explora Scout→Schedule→Stats→Playbook seguidos vería 4 wizards completos seguidos), se implementó **una sola tarjeta compacta** (`Dialog` centrado, no bloqueante) la primera vez real que se abre cada módulo — se cierra con el botón, la X, o tocando fuera, y no vuelve a aparecer salvo que se pida explícitamente desde Ajustes.

- **Contenido, no repetido a ciegas**: se verificó primero qué cubre ya el tutorial principal — para coach solo cubre Scout/Club/Reports/Settings, para jugadora solo Reports/Wellness/Schedule. **Stats y Playbook no se mencionan en absoluto** en el onboarding principal, en ningún rol — son información genuinamente nueva, no redundante. Scout y Club sí solapan parcialmente con el tutorial principal, así que su tarjeta es más breve/complementaria (menciona el flujo My Scout→Film Room→Game Plan de pasada, no lo repite entero).
- **`client/src/lib/onboarding-state.ts`** ampliado: `MODULE_INTRO_KEYS` (scout/schedule/stats/playbook/club), `hasSeenModuleIntro`/`markModuleIntroSeen`/`resetModuleIntros`, mismo patrón de flag en `localStorage` por usuario que ya usaba el onboarding principal, independiente de aquel (un usuario puede tener el tutorial de bienvenida hecho y aun así no haber abierto nunca Stats).
- **`client/src/lib/module-intro-content.ts`** (nuevo): registro de copy por módulo × audiencia (staff/jugadora) en los 3 idiomas — Stats es la misma pantalla para ambos roles (verificado, sin ramificación por rol en `Stats.tsx`), Scout/Schedule/Playbook sí difieren por rol, Club es solo de staff (jugadoras nunca llegan a `ClubManagement.tsx`, guard existente `profile?.role === "player"`).
- **`client/src/components/ModuleIntroCard.tsx`** (nuevo): el componente visual — `Dialog` de shadcn/ui (no `Sheet`, para que se vea bien centrado tanto en el layout de móvil como en el de escritorio sin lógica responsive adicional).
- **Cableado en un único punto real**: `ModuleGate.tsx` ya envolvía los 4 módulos con gate de activación (`scout`/`schedule`/`stats`/`playbook`) — se amplió ahí mismo para montar la tarjeta cuando el módulo está habilitado, en vez de repetir la lógica en cada página. Nuevo prop `showIntro` (default `true`) para el único caso real donde el mismo `moduleKey` gatea dos pantallas distintas: `/player/wellness` reutiliza `moduleKey="schedule"` solo para el control de acceso, pero es una pantalla distinta — se le pasa `showIntro={false}` explícitamente para que no duplique la tarjeta de Schedule. `Club` no pasa por `ModuleGate` (no es un módulo desactivable), se montó directamente en `ClubManagement.tsx`.
- **Reversible/repetible desde Ajustes**: nueva sección "Tutoriales" en `Settings.tsx` (coach, y jugadoras en modo preview) y `PlayerHomeSettingsStub.tsx` (jugadoras reales — son pantallas de ajustes distintas, verificado por qué ruta usa cada una realmente vía `ModuleHeader.tsx::settingsHref`) — botón "Ver los tutoriales otra vez" que limpia los flags de módulo y fuerza un replay del onboarding principal, con recarga a `/home`.

**Bug real encontrado y cerrado al construir el botón de "ver otra vez", no anticipado**: `shouldOfferOnboarding()` nunca ofrece el onboarding principal a cuentas creadas antes del 12 de abril de 2026 (`ONBOARDING_V2_LAUNCH_ISO`), sin excepción — solo mira la fecha de creación de la cuenta, nunca el flag de "completado". Sin corregir esto, el botón de Ajustes habría sido un no-op silencioso para cualquier cuenta antigua (probablemente incluida la del propio Pablo). Corregido con un flag de "replay solicitado" de un solo uso (`resetOnboarding()` lo activa, `shouldOfferOnboarding()` lo consume y lo borra) que se comprueba **antes** que la fecha de lanzamiento — bypassa la puerta de fecha solo cuando alguien lo pide explícitamente, sin cambiar el comportamiento por defecto para cuentas nuevas ni legacy.

**Nueva suite de pruebas** (`onboarding-state.test.ts`, 10 pruebas) — cubre explícitamente el bug anterior (cuenta legacy nunca se ofrece de entrada, pero `resetOnboarding()` sí fuerza el replay una vez y se consume solo una vez) y los flags de módulo (por módulo, por usuario, limpieza conjunta). Nota técnica: este repo no tiene `jsdom`/entorno de navegador configurado en vitest (verificado con `node -e "console.log(typeof localStorage)"` → `undefined`) — el archivo de test stubea un `localStorage` mínimo en memoria antes de importar el módulo, para ejercitar la lógica real en vez de las ramas de fallback `catch`.

Verificado: `npm run check` limpio, `npx vitest run` 16/16 archivos, 234/234 pruebas (10 nuevas). Smoke test del build sin errores de consola en la pantalla pública (Login) — no se pudo probar interactivamente el flujo autenticado completo (Home → abrir cada módulo → ver la tarjeta) por no disponer de credenciales reales; verificado por lectura de código, tipos y tests unitarios, no asumido.

## 45. Recordatorios diarios en iOS — permiso de notificaciones + horario configurable por el head coach (2026-09-14)

Pablo pidió 3 cosas: (1) que la app pida permiso de notificaciones en iOS, (2) que en My Club el head coach pueda fijar una hora para el aviso de "revisa los scout reports" y otra para "rellena el wellness", y delegó explícitamente 2 decisiones: (3) ¿wellness todo junto o separar la calidad del sueño (mañana) del resto (antes de dormir)?, y la hora por defecto, con su propia preferencia ya apuntada: "todo a la misma, a partir de 21:30 o 22:00".

### 45.1. Decisión de arquitectura (técnica, criterio propio): notificaciones locales, no push

Auditado antes de tocar nada: no existía ninguna infraestructura de notificaciones en el repo (`@capacitor/local-notifications`/`@capacitor/push-notifications` no instalados, sin claves APNs, `CLAUDE.md` ya documentaba esto como pendiente #5). Push remoto exigiría un backend propio de envío + certificados/claves de Apple Developer que Pablo no tiene montados — coste real, no justificado para 2 avisos diarios a una hora fija. **Notificaciones locales** (`@capacitor/local-notifications`, programadas en el propio dispositivo, sin servidor ni APNs) encajan exactamente con el caso de uso (hora fija diaria, repetible) y no dependen de nada que Pablo tenga que gestionar en Apple Developer. Instalado `@capacitor/local-notifications@8.3.1` (compatible con el core `@capacitor/core@8.3.2` ya instalado), registrado en el proyecto nativo vía `npx cap sync ios` (actualiza `ios/App/CapApp-SPM/Package.swift`, gestionado por Capacitor CLI, no a mano).

**Alcance deliberado**: solo jugadoras reciben estos 2 avisos (revisar informes ya publicados, rellenar wellness) — los entrenadores no los necesitan, ninguno de los dos flujos les aplica. Solo iOS nativo (`Capacitor.getPlatform() === "ios"`) — en Mac Catalyst/web todas las funciones exportadas hacen no-op limpio, verificado con pruebas reales (ver 45.4), no asumido.

### 45.2. Las 2 decisiones delegadas por Pablo, con criterio propio

**Wellness: todo junto, sin separar.** Verificado primero qué pregunta exactamente cada campo antes de decidir (`client/src/lib/wellness.ts`, `wellness_tooltip_sleep` en `locales/es.ts`): los 4 campos (`sleep_quality`, `energy_level`, `muscle_soreness`, `mental_readiness`) ya se guardan como una sola fila por día con un único `submitted_at` — es decir, ya estaba diseñado como un cuestionario de bienestar cohesionado de una sola vez (patrón estándar en deporte de equipo, tipo Hooper-Mackinnon), no como piezas independientes. Separar "calidad del sueño" a un aviso de mañana y el resto a uno de noche introduciría una segunda notificación diaria y una segunda apertura de la app por el mismo motivo — fricción real que reduce el cumplimiento en este tipo de cuestionarios (bien documentado en monitorización de atletas: más avisos, menos respuesta), a cambio de una ganancia marginal en un solo campo. Coincide además con el propio criterio de Pablo ("que es mas comodo"). No se tocó `WellnessStandalone.tsx` — ninguna de las preguntas actuales, incluida "¿Cómo dormiste?" (pasado, ya pensada para poder responderse en cualquier momento del día, no solo nada más despertar), necesitaba cambiar para esto.

**Hora por defecto: 21:30, la misma para los 2 avisos.** Dentro del rango que ya proponía Pablo (21:30-22:00), 21:30 dado algo más de margen antes de una hora de sueño típica que 22:00, sin razón real para diferenciarlo del aviso de scout reports (ambos son recordatorios diarios de hábito, no ligados a un evento concreto como un partido). `DEFAULT_NOTIFY_TIME = "21:30"` (`client/src/lib/local-notifications.ts`) — nunca se persiste como tal en la fila del club (columnas `NULL` hasta que un head coach las toca), solo se resuelve en el momento de leer/mostrar/programar, así el valor por defecto puede cambiarse centralmente en el futuro sin migrar datos.

### 45.3. Implementado

- **Esquema**: `clubs.scout_reports_notify_time`/`clubs.wellness_notify_time` (`varchar(5)`, "HH:MM", nullable = usa el valor por defecto) — migración `0008_club_notify_times.sql`, aplicada directamente contra producción antes de escribir código contra ella (mismo protocolo que las migraciones 0006/0007). Validación `zNotifyTime` (regex `HH:MM` 24h) en `shared/club-context.ts`, reutilizada en `patchClubBodySchema`.
- **`GET`/`PATCH /api/club`**: ambos campos añadidos a la respuesta y al cuerpo aceptado, mismo gate de permisos ya existente (`userCanManageClub`) — sin endpoint nuevo, se apoya en la infraestructura de "ajustes de club" ya construida para `disabledModules`/`reportMode`.
- **My Club → nueva sección "Horario de los avisos"** (`ClubManagement.tsx`, dentro de la pestaña Club, junto al resto de ajustes de contexto): 2 `<input type="time">`, uno por aviso, con nota del valor por defecto. Mismo patrón de guardado optimista (`usePatchClub`) que el resto de los ajustes del club — sin botón "guardar" aparte.
- **`client/src/lib/local-notifications.ts`** (nuevo): `isNativeNotificationsSupported()`, `getNotificationPermissionState()`, `requestNotificationPermission()` (dispara el diálogo real del sistema — solo se llama tras un toque explícito, nunca al abrir la app), `scheduleDailyReminders()` (programa/reprograma los 2 avisos por id estable — cancela y vuelve a crear, así cambiar la hora en My Club no duplica notificaciones; no pide permiso por su cuenta, si aún no está concedido no hace nada) y `cancelDailyReminders()` (infraestructura lista, sin UI que la exponga todavía).
- **`NotificationsPrimingCard.tsx`** (nuevo): tarjeta de "priming" — explica los 2 avisos con la hora real configurada por el club antes de disparar el diálogo nativo de iOS (Apple HIG: nunca lanzar el permiso del sistema sin contexto). Se muestra una sola vez por jugadora (`hasSeenNotificationsPriming`/`markNotificationsPrimingSeen`, mismo patrón de flag en `localStorage` que el resto del onboarding) y solo si el estado real de permiso en el sistema es `"prompt"` (si ya estaba concedido o denegado de antes, no hay nada que "primar", se marca vista sin mostrarla).
- **Cableado en Home** (`HomeMobile.tsx` y `HomeDesktop.tsx` — las 2, no solo una: `Home.tsx` elige entre ambas por ancho de viewport vía JS, un iPad de una jugadora podría caer en cualquiera de las 2): la tarjeta de priming se monta una vez para `mode === "player"`, y un `useEffect` reprograma los 2 avisos en cada visita a Home si el permiso ya estaba concedido de antes — así un cambio de hora hecho por el head coach en My Club se refleja sin que la jugadora tenga que repetir el permiso.

### 45.4. Verificación

- `npm run check` limpio, `npx vitest run` 17/17 archivos, 241/241 pruebas (7 nuevas en `local-notifications.test.ts`) — cubren explícitamente que las 5 funciones exportadas hacen no-op limpio y seguro fuera de iOS nativo (confirmado con `node -e` antes de escribir el test que `@capacitor/core` no lanza sin puente nativo, no asumido), y el valor/formato de `DEFAULT_NOTIFY_TIME`.
- `npm run build` (build real de producción, no solo `tsc`) completado sin errores — confirma que el nuevo plugin no rompe el bundle de cliente ni de servidor.
- `npx cap sync ios` ejecutado y verificado: `Package.swift` actualizado correctamente con la dependencia del plugin.
- Smoke test interactivo en el navegador embebido sin errores de consola (pantalla pública, mismas limitaciones de credenciales que en la sección 44 para el flujo autenticado completo).

### 45.5. Efecto secundario real encontrado y corregido en el propio proceso: `npx cap sync ios` pisó un cambio local de Pablo, sin commitear

Antes de este trabajo, `ios/App/App.xcodeproj/project.pbxproj` ya aparecía como modificado sin commitear (`PRODUCT_BUNDLE_IDENTIFIER = com.pablomgz.ucore`, distinto del valor en el último commit) — trabajo en curso de Pablo, junto con `capacitor.config.ts` (`appId` sin commitear cambiado a `com.ucorebasketball.app`) y 2 archivos nuevos sin trackear (`appstore-listing-copy.md`, `client/public/privacy-policy.html`), consistente con preparación real de la ficha de App Store, ajena a esta sesión — no tocados a propósito hasta este punto.

`npx cap sync ios` regenera `PRODUCT_BUNDLE_IDENTIFIER` a partir del `appId` de `capacitor.config.ts` — al ejecutarlo, sobrescribió el valor local de Pablo (`com.pablomgz.ucore`) con el de su propio `capacitor.config.ts` sin commitear (`com.ucorebasketball.app`), sin que esto tuviera nada que ver con el plugin de notificaciones. Detectado revisando el diff real tras el sync (no asumido limpio) y **revertido de inmediato** solo en esa línea, dejando intacto el resto del cambio real del plugin (`Package.swift`). `ios/App/App.xcodeproj/project.pbxproj` queda hoy exactamente igual que antes de tocar nada.

**Nota para Pablo, no resuelta, no es mía para decidir**: el bundle id ya estaba inconsistente entre 3 sitios distintos antes de esta sesión — el del último commit (`com.pablomgz.ucore` en Xcode, `com.ucore.app` en `capacitor.config.ts`, ya discordantes entre sí) y su edición local sin commitear de `capacitor.config.ts` (`com.ucorebasketball.app`, aún no aplicada a Xcode). Cuál es el id correcto/definitivo para el envío a App Store es una decisión suya, ligada a lo que tenga registrado en Apple Developer — no se ha tocado ni asumido nada al respecto.

Con esto: permiso de notificaciones pedido con contexto (nunca en silencio), horario configurable de verdad por el head coach, wellness sin fragmentar (criterio propio justificado), y el trabajo en curso de Pablo sobre el bundle id intacto.

## 46. [SIN TOCAR — hallazgo real pendiente de decisión de Pablo] Los overrides de un entrenador (picks de "reemplazar alternativa") podrían no llegar nunca al informe publicado

Al retomar el pendiente #11 de `CLAUDE.md` ("Film Room: flujo de resolución de discrepancias poco claro para coaches nuevos") para escribir un texto de ayuda real en `DiscrepancyPanel` (`FilmRoom.tsx`), hacía falta saber **qué pasa de verdad cuando una discrepancia se "resuelve"** antes de escribir nada — y siguiendo el rastro del código (no asumido, cada paso verificado por lectura directa) apareció algo más gordo que un problema de copy. **No se ha tocado ningún código para esto** — es un hallazgo, no un fix, precisamente porque no tengo confianza suficiente para decidir cuál de las 2 lecturas de abajo es la intencionada.

**Lo verificado, paso a paso:**
1. Cuando un coach elige una alternativa distinta en el sheet de "Alternativas del motor" (`ReportSlidesV1.tsx::pickAlternative`), eso crea una fila en `report_overrides` (`POST /api/players/:id/overrides` → `storage.upsertReportOverride`) — **no toca `players.inputs`** (verificado: `players.inputs` solo se escribe vía `PATCH /api/players/:id`, que es un formulario distinto, de datos de scouting/roster, no de picks de alternativas).
2. `FilmRoom.tsx::DiscrepancyPanel` detecta y muestra cuando 2+ entrenadores tienen overrides distintos en el mismo campo (`hasDiscrepancy`) — pero **no ofrece ninguna acción para resolverlo**, ni "adoptar la versión de fulano", ni "fusionar". Solo lo muestra.
3. Al pulsar "Publicar → Game Plan" (`POST /api/players/:id/game-plan`), el servidor llama a `storage.mergeAndClearScoutVersions(playerId)`, que hace literalmente `DELETE FROM report_overrides WHERE player_id = ...` y `DELETE FROM player_scout_versions WHERE player_id = ...` — **sin leer su contenido antes, sin escribir nada a partir de ellos**. El comentario de código junto a esa función dice *"canonical inputs are already in players.inputs (set by head_coach during the merge/publish step)"* — pero no existe ningún paso, en ningún sitio del código, que escriba a `players.inputs` a partir de `report_overrides` ni de `player_scout_versions`. Ese comentario describe un paso que no está implementado (o dejó de estarlo).
4. Lo que ve la jugadora tras publicar (`ReportSlidesV1.tsx` vía `PlayerOpponentReportRoute`/`PlayerReportV4Route`) es `ensamblarReporteParaTexto(playerInputToMotorInputs(player.inputs), ...)` más `GET /api/players/:id/overrides` — que en ese momento **ya está vacío**, porque el paso 3 acaba de borrarlo todo.

**Conclusión del rastreo, no una opinión**: tal como está el código hoy, ningún pick de "reemplazar alternativa" de ningún entrenador llega jamás al informe que ve la jugadora — el informe publicado es siempre la salida por defecto del motor (más los patrones promocionados de Nivel B, que sí son permanentes por diseño, sección 39-41), nunca el pick manual puntual de un entrenador para ESE informe en concreto.

**Dos lecturas posibles, y por qué no elijo una sin preguntar:**
- **(A) Es un bug real**: el nombre mismo de la pantalla ("Film Room · resolver discrepancias") y el hecho de que exista un `DiscrepancyPanel` sugieren que el producto se pensó para que los entrenadores lleguen a un consenso y ESE consenso se publique. Si es así, falta un paso real: aplicar el override ganador (o el del entrenador que publica) a `players.inputs` — o a algún otro campo — antes de borrar `report_overrides`.
- **(B) Es el diseño correcto**: los overrides son una capa de "qué alternativa prefieres tú, ahora mismo, para revisar" — sirven para que cada entrenador vea su propia lectura del informe y para alimentar la detección de patrones de Nivel B (que si un patrón se repite entre suficientes entrenadores, SÍ se vuelve permanente, de forma reversible, vía el panel de calibración). El informe publicado usa siempre la salida calibrada del motor, nunca un pick manual de una sesión de revisión puntual, y "resolver una discrepancia" significa literalmente eso: que los entrenadores se pongan de acuerdo verbalmente y dejen de elegir alternativas distintas, no que el sistema fusione nada. Bajo esta lectura, nada está roto — el `DiscrepancyPanel` solo necesitaría un texto que lo explique así.

Dado que esto es exactamente del tipo que CLAUDE.md pide comprobar antes de tocar ("decisiones de producto no triviales... cualquier cosa donde te sorprendas pensando 'esto seguramente ya se decidió de alguna forma'") — y "el decantador"/el sistema de overrides fue diseñado explícitamente en conversaciones anteriores de Claude Desktop (abril, referenciadas en las secciones 38-39) — esto necesita la misma pregunta a Claude Desktop antes de decidir cuál de las 2 lecturas es la real, en vez de que yo elija una y monte un fix (o un texto de ayuda) sobre la lectura equivocada.

**No se ha tocado nada**: ni `FilmRoom.tsx`, ni `mergeAndClearScoutVersions`, ni el pendiente #11 de `CLAUDE.md` (se deja tal cual, con una nota apuntando aquí). El único cambio de esta sección es documentación.

## 47. Cerrado — el hallazgo de la sección 46 SÍ era un bug real, confirmado por Pablo vía investigación de Claude Desktop (2026-09-15)

Pablo pasó las 3 preguntas de la sección 46 a Claude Desktop y trajo la respuesta, con cita literal del diseño original (conversaciones "App en replit", 2026-04-03, y "Rediseño de reports: estructura de 3 capas", 2026-04-18):

> *"cuando un coach cambia algo, lo que cuenta es eso y se mide eso en cuanto a discrepancias con los demás, y después entre todos se aprueba una versión final antes de publicarlo, cualquier entrenador puede decidir publicar. cuando se publica, la opción del que publica se considera la versión final y esto se memoriza para los futuros cambios en soft program y para las stats del master."*

Y, sobre si los overrides tocan los datos canónicos: **no, nunca** — *"no quiero que al modificar nada de esto cambie los inputs, solo el report total"*. Esto confirma la lectura (A) de la sección 46: es un bug real, no un diseño distinto. Los 3 puntos del diseño original:
1. `report_overrides` es una capa persistente, aplicada en tiempo de renderizado sobre los datos canónicos — nunca los modifica. **Esto ya estaba bien implementado.**
2. Al publicar, el conjunto de overrides del entrenador que publica se congela como la versión final — no es un preview efímero. **Esto NO estaba implementado: se borraba entero.**
3. Ese conjunto también alimenta el aprendizaje (Nivel A soft por coach, Nivel B hard agregado — este último ya construido, secciones 38-41). **Borrarlo al publicar también rompía esto silenciosamente.**

### 47.1. Implementado

- **`server/storage.ts`**: `mergeAndClearScoutVersions()` se queda EXACTAMENTE igual (borra versiones + overrides) pero se documenta correctamente como lo que de verdad es — el reset de `POST /api/players/:id/unpublish` ("empezar de cero"), su único llamador real; el comentario anterior decía que se llamaba "al publicar", lo cual no era cierto. Nueva función hermana `clearScoutVersionsAfterPublish()`: borra solo `player_scout_versions` (los borradores, ya cumplieron su función), **no toca `report_overrides`**.
- **`server/routes.ts::POST /api/players/:id/game-plan`**: cambiado `mergeAndClearScoutVersions` → `clearScoutVersionsAfterPublish`. Es el único cambio de comportamiento real de publicación — los overrides ya no desaparecen al publicar.
- **`server/routes.ts::GET /api/players/:id/overrides`** (el bug estructural más directo, independiente incluso del borrado): antes filtraba SIEMPRE a `coachId === quien pregunta` — correcto para un entrenador revisando su propio borrador, pero roto para una jugadora viendo el informe publicado, porque una jugadora nunca es "coachId" de ningún override — esa comparación daba 0 filas siempre, sin importar lo que hubiera elegido cualquier entrenador. Corregido: si el jugador está publicado, devuelve los overrides de `coachId === players.published_by` (el entrenador que publicó, versión congelada, igual para cualquiera que pregunte); si no, sigue devolviendo los del propio usuario que pregunta (sin cambios, sigue siendo el "mi borrador" de siempre).
- **`GET /api/players/:id/approval-status` y `GET /api/film-room`**: `hasDiscrepancy` ahora es `false` cuando el jugador ya está publicado — consecuencia directa de que los overrides ya no se borran: sin este ajuste, un informe ya publicado (y por tanto ya resuelto, congelado) seguiría mostrando "⚠ conflicto" para siempre en Film Room, lo cual sería confuso y falso, no una decisión de producto nueva.

### 47.2. Deliberadamente NO tocado en esta pasada (fuera de lo que Pablo confirmó)

- **`player_scout_versions` en publish**: sigue borrándose (sin cambios) — Pablo no mencionó esta tabla, solo `report_overrides`. Efecto secundario ya existente, no introducido aquí: un jugador recién publicado desaparece de la lista de Film Room (`submittedCount > 0` dejaría de cumplirse) hasta que alguien vuelva a enviar una versión — la rama de UI "✅ En Game Plan" de `FilmRoomCard.tsx` casi nunca se alcanza en la práctica por este motivo. No confirmado por Pablo como bug, no tocado.
- **Comentario incorrecto en el propio código de `unpublish`**: el handler de `POST /api/players/:id/unpublish` dice en un comentario "Clear all scout versions and approvals" pero el código real nunca borra `report_approvals` — hallazgo menor encontrado de pasada, no pedido, no corregido aquí.
- **Nivel A (soft, por coach)**: sigue fuera de alcance (ya lo estaba, secciones 17.3/39). La cita de Pablo lo menciona ("se memoriza... para los futuros cambios en soft program") pero construirlo no es parte de este cierre — este cierre solo asegura que el dato (`report_overrides` persistente, con `replacementKey` estable) queda disponible para cuando se construya.

### 47.3. Verificación

`npm run check` limpio, `npx vitest run` 17/17 archivos (sin cambios — este fix es 100% servidor, y este repo no tiene tests de servidor, ver nota abajo), `npm run build` (producción real) sin errores. **No se pudo hacer un smoke test interactivo end-to-end** (2 entrenadores en discrepancia → publicar → verificar como jugadora) por 2 motivos reales, no excusas: no hay credenciales de prueba disponibles en esta sesión, y la base de datos de producción no tiene datos reales todavía (verificado por consulta directa: 0 filas en `players.published=true`, `report_overrides`, `player_scout_versions`). Verificado en su lugar por lectura exhaustiva de cada paso del pipeline (citado arriba con archivo y línea) y por `npm run check`/`npm run build`. Este repo no tiene infraestructura de test de servidor (`vitest` solo cubre `client/src/**`, por el `root` configurado en `vite.config.ts`) — no se ha montado una nueva para este fix puntual, sería una inversión de infraestructura desproporcionada para el alcance de este cambio; la cobertura real de este pipeline sigue dependiendo de revisión de código cuidadosa + verificación manual cuando haya datos reales.

Con esto, el diseño original de "el decantador" queda implementado tal como se pensó en abril: el pick de un entrenador al publicar es la verdad, persistente y disponible para el aprendizaje futuro — no un preview que se esfuma.

## 48. Resolución de discrepancias campo a campo — un supervisor puede adoptar el pick de un compañero antes de publicar (2026-09-15)

Pregunta directa de Pablo tras cerrar la sección 47: *"¿has tenido en cuenta las fases de override individuales de cada coach al revisar su report y discrepar con el motor, y después al ponerse en común con el resto del staff que se seleccionará un report (o su render overrided) ganador?"* — y, en un segundo mensaje, más preciso: *"podria darse el caso de que al seleccionar un report ganador entre todo el staff, que entre todos lleguen a la conclusion de que el report debe de tener algunas partes de cada una de las propuestas. [...] habria que dar una vez mas opcion de override a los supervisores no?"*

**Respuesta honesta, no defensiva**: la sección 47 dejó bien el mecanismo de fondo (quien publica, su override actual se congela como versión final — eso coincide con la cita literal de abril), pero no daba ninguna forma real de **llegar** a ese estado combinado cuando el "ganador" no es la propuesta íntegra de un solo entrenador, sino una mezcla de varias. `DiscrepancyPanel` solo mostraba que había conflicto, no permitía resolverlo desde la app — un entrenador tendría que replicar a mano, campo por campo, lo que otro compañero había elegido, adivinando cuál era el `replacementValue` exacto.

**Implementado, reutilizando la arquitectura existente sin inventar ningún mecanismo nuevo**: no hacía falta ninguna tabla ni endpoint nuevo — `POST /api/players/:id/overrides` (el mismo que usa el picker de alternativas de siempre) ya admite exactamente esto: un campo, un `coachId`, un pick. Lo único que faltaba era la UI para dispararlo desde el propio panel de discrepancias, con los datos del compañero ya rellenados.

- **`GET /api/players/:id/approval-status`**: la lista de `overrides` ahora incluye también `replacementKey`, `archetypeKey` y `locale` (antes solo `action`/`replacementValue`/scores) — sin esto, "adoptar" el pick de un compañero habría guardado un override incompleto, sin el `OutputKey` estable que necesita Nivel B para seguir detectando patrones correctamente.
- **`DiscrepancyPanel` (`FilmRoom.tsx`) rediseñado**: ahora agrupa cada campo en conflicto por el **valor real elegido** (mismo criterio de agrupamiento que `detectPatterns()`: `replacementKey ?? replacementValue ?? action`, no el texto crudo), muestra el nombre de cada compañero que eligió cada opción (`userDisplayLabel`, mismo helper que usa `ClubManagement.tsx`), resalta cuál es la opción del propio usuario actual, y — solo para supervisores — un botón "Usar esta" por cada alternativa ajena que llama a `useSetReportOverride` (el mismo hook que ya usa el picker de alternativas) con los datos exactos de ese compañero, bajo el `coachId` del propio supervisor.
- **Permiso**: gateado a `canAccessCalibrationPanel` (head_coach/master siempre, coach con `reportPublishAccess` delegado) — la misma capability que ya controla el panel de Calibración/Nivel B, coincide con la palabra que usó Pablo, "supervisores", sin inventar un permiso nuevo.
- **Privacidad**: mostrar nombres reales aquí es correcto y no contradice el principio de anonimato de la sección 17.2 — ese principio es para el panel de Calibración (patrones agregados entre todo el club, cross-jugadora), aquí es una discusión de staff sobre un informe concreto de una jugadora concreta, donde el equipo ya sabe quién es quién y necesita saberlo para decidir.
...
## 52. Preparación para App Store — bloqueo de registro público de nuevos clubes (2026-09-15)

Pablo, mandato directo: sin sistema de pagos todavía y con la app a punto de subir a la App Store, cualquiera que la descargara y se registrara como "Head Coach" conseguía un club nuevo, gratis y totalmente funcional. Pidió bloquear el uso a desconocidos (pueden descargarla, no pueden usarla) hasta tener pagos.

**Verificado primero qué hacía falta bloquear de verdad, no asumido**: el flujo de invitación a un club existente (`JoinClub.tsx`, roles coach/jugadora) ya estaba controlado — exige un token de invitación real generado por un head_coach existente, no lo puede alcanzar nadie de fuera. El único hueco real es el auto-alta de club nuevo para quien se registra directamente como "Head Coach" desde `/login` — eso es lo único que se cierra aquí, nada más.

**Implementado en el servidor, no solo escondido en el cliente** (esconder el botón en `Login.tsx` no habría bastado — cualquiera podría llamar a la API directamente): `GET /api/club` (`server/routes.ts`), el único sitio real donde un `head_coach` obtiene un club nuevo, ahora comprueba el email contra `HEAD_COACH_SIGNUP_ALLOWLIST` (variable de entorno de Railway, lista separada por comas) antes de crear el club — si no está en la lista, devuelve `403 signup_closed` en vez de crear nada. `master` nunca se restringe (rol de confianza interna). Si la variable de entorno no está definida en algún entorno, cae a una lista de respaldo con solo el email de Pablo — falla cerrado, nunca abierto por descuido.

- **`client/src/App.tsx::ClubSecurityGate`**: nueva rama que distingue este caso (`signupClosed`) del "no club found" ya existente (sección 49) — mensajes distintos a propósito: a alguien esperando una invitación real se le dice que la pida; a un desconocido rechazado se le dice que el registro público no está abierto todavía y un email de contacto, no "pide una invitación" (que sería confuso/falso para alguien que nunca va a recibir una).
- **Variable configurada en producción**: `HEAD_COACH_SIGNUP_ALLOWLIST=pablomgz@hotmail.com` (Railway, servicio `u-scout` producción) — verificado por consulta directa a Supabase antes de escribir el valor: es el único email real con cuenta `head_coach` que existe hoy (ver sección 53 para el resto del panorama de datos reales). Ampliar la lista en el futuro (nuevas cuentas de prueba, más clubes) es cambiar esta variable en Railway, sin necesitar ningún deploy de código.
...
## 55. Eliminar/banear miembros unificado en una sola acción reversible + aviso real al expulsado (2026-09-15)

Pablo, con contexto real de producto (están haciendo tryouts, invitarán a jugadoras que eventualmente saldrán del club): *"teniamos en specs opcion de eliminacion y baneo pero creo que podemos agruparlas en eliminar, porque la intencion basica es expulsarles del flujo de informacion del equipo, y eliminar toda la informacion que ya tuviesen, [...] enviar una query a su telefono para que deletee todos los datos en local [...], expulsarle y que le salga un aviso, y que pase a quedarse [...] tienes cuenta de staff o jugador pero no perteneces a ningun equipo."*

**Verificado primero qué existía de verdad, antes de rediseñar:**
- `DELETE /api/club/members/:id` — borrado duro (fila desaparece del todo, sin rastro).
- `PATCH .../ban` / `.../unban` — marca reversible (`status: 'banned'`, fila se conserva).
- El aviso al expulsado **solo existía, a medias, para el caso de baneo**: `ClubSecurityGate` (`App.tsx`) ya limpiaba `localStorage`/caché de React Query al detectar un 403 "banned" — pero lo hacía completamente en silencio (sin ninguna pantalla, cerraba la sesión al instante) y **el borrado duro (`DELETE`) no disparaba nada de esto en absoluto** — un miembro eliminado con borrado duro se quedaba con un 404 genérico "No club found" (el mismo mensaje que un usuario nuevo esperando invitación), sin limpieza local y sin ninguna explicación real de qué había pasado. Justo el hallazgo que Pablo sospechaba.

**Decisión de implementación (técnica, no pedida explícitamente pero justificada aquí)**: unificar usando el mecanismo de **baneo** (reversible) como el único camino real, no el borrado duro — valioso de verdad en un contexto de tryouts, donde eliminar a la jugadora equivocada por error es plausible y poder deshacerlo sin perder el historial importa. El borrado duro (`DELETE`) se deja intacto en el servidor por si hace falta en el futuro, pero deja de estar cableado a la UI.

**Implementado:**
- **`client/src/pages/scout/ClubManagement.tsx`**: los 2 botones/acciones distintas ("Eliminar" con borrado duro, "Banear" con marca reversible) se convierten en **un único botón/elemento de menú** por fila de miembro — "Eliminar" cuando está activo, "Restaurar" cuando ya está eliminado (mismo patrón en las 2 variantes de fila, `staff` y `player`). Un solo diálogo de confirmación, con texto que explica las 2 consecuencias reales ("perderá el acceso al instante, sus datos de este club se borrarán de su dispositivo, puedes restaurar su acceso más tarde si fue un error") en vez de mostrar solo el nombre sin contexto. Limpieza de código muerto: `useDeleteClubMember`/`delMember` (hook, prop, tipo, estado del diálogo de baneo separado) retirados de este archivo al dejar de usarse.
- **`client/src/App.tsx::ClubSecurityGate`**: rediseñado para los 3 casos de "sin acceso real" (baneado/eliminado, sin club, registro cerrado) de forma consistente:
  - El borrado de datos locales (`wipeLocalClubData()`, extraído a función propia) ahora se dispara para **los 3 casos**, no solo el de baneo — inofensivo para los otros 2 (nunca tuvieron datos de club que borrar).
  - Ya **no se fuerza el cierre de sesión automático e instantáneo** para el caso de baneo/eliminación — antes eso pasaba en silencio, sin que la persona expulsada llegara a ver ningún mensaje. Ahora se muestra una pantalla real explicando qué pasó ("Ya no perteneces a este club — tu cuerpo técnico te ha retirado del club, sus datos se han borrado de este dispositivo, si es un error pide que te reactiven"), con un botón manual de cerrar sesión — mismo patrón ya usado para los otros 2 casos (secciones 49 y 52), consistente en los 3.
- **Traducciones revisadas de paso** (petición explícita de Pablo, "revisa traducciones a la vez q la cosmetica"): `club_status_banned` decía "Suspendido"/"Banned"/"已封禁" — desalineado con el nuevo concepto unificado, corregido a "Eliminado"/"Removed"/"已移除". `club_unban` decía "Reactivar"/"Unban"/"解除封禁" (con la palabra "ban" filtrándose incluso al inglés/chino pese a que ya no se expone como "baneo" en ningún sitio de la UI), corregido a "Restaurar"/"Restore"/"恢复". La clave `club_ban` queda huérfana (cero usos reales, verificado por grep) y se elimina de los 3 idiomas.

**Aclaración explícita a Pablo, verificada, no solo prometida**: el flujo de invitación (`JoinClub.tsx`, `POST /api/club/invitations/:token/accept`) es un endpoint completamente aparte del gate de registro público de la sección 52 (`GET /api/club`) — no comparten ni una línea de código, no se ve afectado en absoluto. Jugadoras y staff invitados por enlace real siguen entrando exactamente igual que siempre.

Verificado: `npm run check` limpio, `npx vitest run` 17/17 archivos, `npm run build` (producción real) sin errores. Smoke test en navegador embebido (pantalla pública) sin errores de consola. No se pudo probar el flujo completo de eliminación/restauración de un miembro real de forma interactiva (exige sesión autenticada de head_coach con al menos 2 miembros reales en el club) — verificado por lectura exhaustiva del código y de la cadena de detección de errores cliente↔servidor.

## 56. Gate de registro público preparado para "levantar el bloqueo" cuando haya pagos, sin acoplarse a ningún procesador (2026-09-15)

Pablo, tras confirmar la cuenta de prueba `ucore.qa.headcoach@test.com`: *"eso de no poder crear head coaches en el futuro esta mal, una vez desbloqueemos la app, aquellos q paguen deberan poder crearse cuenta head coach, plantea como dejar eso listo solo para cuando tengamos pagos levantar el bloqueo y yasta"*.

**Investigación previa (protocolo de "esto huele a ya decidido antes")**: la tabla `subscriptions` existe de verdad en Supabase producción (0 filas) pero no estaba declarada en `shared/schema.ts` ni referenciada en ningún endpoint — señal fuerte de diseño previo no wireado, igual patrón que "el decantador" en sesiones anteriores. Se le pidió a Pablo que consultase Claude Desktop antes de diseñar desde cero. Resultado, con un matiz importante (el modelo cambió a mitad de esa conversación original, hay que quedarse con la versión final):
- El SQL literal de la tabla coincide con el esquema real de producción: `plan` (`basic`/`pro`/`elite`, default `basic`), `max_coaches`/`max_players`/`max_rival_teams` (defaults 3/15/20), `status` (`active`/`cancelled`/`expired`), `expires_at`.
- Modelo de negocio definitivo (una versión anterior, de "el club compra un staff con X/Y slots", quedó invalidada por Pablo dentro de esa misma conversación — *"no queremos vender suscris a los clubes sino a los entrenadores"*): **el head coach es el cliente**, paga su plan directamente, esa compra crea/activa su propio "universo" (club) — no hay aprobación manual de admin/master, es autoservicio. Desde ahí invita a su staff y jugadoras (email vía `Supabase.inviteUserByEmail()` desde servidor, o link `/join/CODE`) — mecanismo ya existente y sin cambios.
- Precios reales (€/mes) y qué diferencia exactamente `pro`/`elite` de `basic` más allá de los 3 números de slots: **no están decididos en ninguna conversación previa** — hueco real de producto, no inventado aquí.

**Implementado (solo el mecanismo, cero acoplamiento a un procesador de pago concreto):**
- **`shared/schema.ts`**: declarada `subscriptions` (Drizzle) reflejando exactamente el esquema real de producción (`id`/`head_coach_id` como `text`, igual que la tabla ya existente — no una migración, solo su descripción para poder consultarla desde el código). Tipos `Subscription`/`InsertSubscription` exportados.
- **`server/storage.ts`**: nuevo método `getActiveSubscriptionForUser(headCoachId)` — devuelve la fila de `subscriptions` con `status = 'active'` y `expires_at` nulo o futuro, si existe.
- **`server/routes.ts`, `GET /api/club`**: el gate de la sección 52 (`isHeadCoachSignupAllowed`, allowlist por variable de entorno) gana una segunda vía, en OR: si existe una suscripción activa para ese usuario, también se le permite auto-crear su club. Hoy `subscriptions` tiene 0 filas, así que esta segunda vía no deja pasar a nadie todavía — el comportamiento visible no cambia.
- **Punto de enganche único para el futuro**: el día que exista un flujo de pago (Stripe/Apple IAP/RevenueCat/…, decisión de producto no tomada aquí), su webhook de "pago confirmado" solo necesita un `INSERT` en `subscriptions` para ese `head_coach_id`. No hace falta tocar el gate, ni la creación de club, ni las invitaciones — ya están listos para leerla. La allowlist de la sección 52 se mantiene para siempre como override manual (cuentas de test, soporte), independiente de si hay pagos activos.

Verificado: `npm run check` limpio, `npx vitest run` 17/17 archivos (241/241 tests), `npm run build` (producción real) sin errores. No se pudo probar el flujo end-to-end de una suscripción activa levantando el bloqueo (exige insertar una fila de prueba en `subscriptions`, que se ha evitado a propósito para no dejar datos de prueba no solicitados en una tabla de producción hasta que Pablo decida usarla) — verificado por lectura exhaustiva del código y por el hecho de que la query nueva sigue el mismo patrón exacto que las demás consultas de `storage.ts` ya probadas en producción (p. ej. `getClubForUser`).

## 57. Migración de Jiangxi a producción real + botón "Calibración" explicado + horarios feos + publicación directa de un solo clic (2026-09-15)

Pablo, tras confirmar el club de pruebas: *"haz la migracion, y dejame Jiangxi ya preparado. otra cosa mas, has implementado en my club un boton q se llama calibracion, nose para q sirve, tmb los botones de horarios de los avisos, los cuadros de las horas quedan feos, corrigelo, y una feature interesante..."* (ver más abajo).

**a) Migración de datos reales (SQL directo en Supabase, verificado antes y después de cada paso — sin migraciones de esquema, solo datos):**
- Estado real confirmado antes de tocar nada: `ucore.qa.headcoach@test.com` (cuenta creada por Pablo, rol concedido en la sección 55/56) todavía no tenía club — nunca había hecho login, así que `GET /api/club` nunca había disparado la auto-creación. Creado directamente (mismo patrón exacto que el endpoint real: club + fila `club_members` head_coach + equipo de sistema "Free Agents", `is_system=true`, necesario porque las fichas sandbox de las cuentas QA vivían en el "Free Agents" de Jiangxi).
- Movidas a ese club nuevo: las 2 cuentas QA (`club_members` de coach y jugadora), su única ficha de jugadora sandbox (`players`, antes en el "Free Agents" de Jiangxi), y sus 5 entradas de wellness (`wellness_entries.club_id`). Las 12 sesiones de calendario de Jiangxi (creadas por el propio Pablo) se dejan tal cual, confirmado explícitamente antes ("las puedes dejar").
- **Verificado el resultado, no solo ejecutado**: Jiangxi queda con 1 solo miembro (Pablo, head_coach) y 308 jugadoras reales del roster WCBA — 0 cuentas ni datos de prueba sueltos. El club de pruebas queda con 3 miembros (head coach + coach + jugadora QA) y su única ficha sandbox, autocontenido.
- Nota técnica encontrada de paso: la tabla `teams` en producción tiene columnas reales (`club_id`, `universe_id`, `is_system`, `name_en`) que **no están** en `shared/schema.ts` — mismo patrón de "tabla real sin cablear" que `subscriptions` (sección 56). No se ha tocado (fuera de alcance de esta pasada), solo se deja anotado.

**b) "No sé para qué sirve" — panel de Calibración sin ninguna explicación en lenguaje llano.** Cierto: el tab y el panel (sección 38-41, "el decantador") solo mostraban jerga técnica ("Sustituciones en las que coinciden varios entrenadores", "Umbral") sin decir qué hace el botón "Hacer permanente" antes de pulsarlo, ni por qué con un solo entrenador activo el panel se ve vacío siempre (el umbral mínimo es 2 entrenadores distintos coincidiendo). Añadida una frase fija, siempre visible (no solo en el estado vacío), en `ClubManagement.tsx::CalibrationPanel`, en los 3 idiomas: explica en una frase qué activa el panel y qué pasa al pulsar el botón. No se ha ocultado el tab — con jugadoras y staff reales entrando ahora en Jiangxi, empezará a llenarse solo.

**b.1) Ampliado el mismo día** — Pablo, tras pedir la explicación por chat: *"puedes explicarlo mejor a fondo en el menu porfa, porque los usuarios no lo van a entender"*. La frase suelta de (b) era insuficiente para usuarios reales sin contexto técnico. Sustituida por una caja destacada (mismo estilo visual que la caja de link de invitación, ya usada en este mismo archivo) con el mecanismo completo en lenguaje llano: qué es (el motor genera el informe solo; a veces un entrenador corrige algo a mano), cuándo aparece un patrón aquí (varios entrenadores distintos corrigen lo mismo en jugadoras parecidas — señal de fallo sistemático, no capricho de una jugadora suelta), un ejemplo concreto, qué hace cada botón ("Hacer permanente"/"Revertir") explicado por separado, y la nota de por qué se ve vacío con un solo entrenador. Verificado `npm run check`/`vitest`/`build` limpios tras el cambio.

**c) Cuadros de hora feos.** Real: los 3 `<input type="time">`/`type="date"` de `ClubManagement.tsx` (horario de avisos de scout reports/wellness, fecha/hora de partido nuevo) nunca declaraban `color-scheme` — sin eso, el navegador dibuja sus controles nativos (icono de reloj, flechas) con su chrome claro por defecto, sin encajar con los 2 temas oscuros de la app (`.dark`, `.theme-oldschool`). Corregido en `index.css`: `color-scheme: light` en `:root`/`.theme-office`, `color-scheme: dark` en `.dark`/`.theme-oldschool` — cambio puramente de chrome nativo, no toca el layout de nada ya estilado a mano. Verificado visualmente en el navegador embebido, en modo oscuro: el icono del reloj pasa de invisible/descolocado a visible y coherente con el resto de la UI.

**d) Publicación directa de un solo clic — sin crear ningún "modo simplificado" nuevo.** Pablo propuso 2 alternativas y se decantó él mismo por la segunda: *"que el que genere un report, una vez hechos los overrides pueda decidir si quiere publicarla directamente o dejarla a la vista del staff para la comparacion, asi sin crear nada nuevo, y con la friccion de un solo clic"*. Investigación antes de tocar nada: hoy el único sitio donde existe un botón "Publicar" real es Film Room (`FilmRoom.tsx`, llama a `POST /api/players/:id/game-plan`) — para llegar ahí, un entrenador tenía que primero "Enviar a sala" desde la pantalla de revisión (`ReportViewV4.tsx`, `mode="coach_review"`) y navegar fuera de esa pantalla. Pero el propio endpoint de publicación **no depende en absoluto** de haber enviado nada a Film Room — su único requisito real es `report_approvals.length >= 1` (ver sección 26); el contenido publicado sale de los datos ya guardados de la jugadora más los overrides del entrenador que publica (`report_overrides`), no de una "versión" enviada a comparación. Es decir: el paso por Film Room era pura indirección de UI, no una dependencia real del motor.

Implementado en `ReportViewV4.tsx` (misma pantalla donde ya se hacen los overrides, sin pantalla ni ruta nueva): junto al botón existente (renombrado "Dejar para comparar con el staff", el flujo de siempre — envía a Film Room, espera a otros entrenadores), un segundo botón "Publicar directamente" que aprueba automáticamente si el entrenador todavía no lo había hecho, y llama al mismo endpoint real (`game-plan`) que ya usaba Film Room — mismo gate del servidor, mismo permiso, cero atajos nuevos de seguridad. Una vez publicado, ambos botones se sustituyen por un aviso claro ("Publicado — visible para las jugadoras"). Funciona igual de bien para un staff de un solo entrenador (siempre elegirá "Publicar directamente", cero fricción extra) que para un staff con varios (pueden seguir usando "Dejar para comparar" para lo importante, y "Publicar directamente" para lo que no necesita segunda opinión) — sin ninguna bifurcación de código por tamaño de staff, ni ajuste que descubrir en Ajustes.

Verificado: `npm run check` limpio, `npx vitest run` 17/17 archivos (241/241 tests), `npm run build` sin errores. Smoke test en navegador embebido (pantalla pública, sin errores de consola) y verificación programática de `color-scheme` en los 4 temas. No se pudo probar interactivamente el botón "Publicar directamente" en sí (exige sesión autenticada de entrenador con una jugadora canónica real) — verificado por lectura exhaustiva del código: mismo endpoint, mismo gate, mismos mensajes de error ya usados y probados en Film Room.

## 58. [SIN TOCAR — decisión de arquitectura aparcada para el futuro] Hosting alternativo a Railway + carga remota vs. web empaquetada en el `.app` de iOS

Pablo pegó el log de consola de un arranque real en iOS vía Xcode ("por si acaso hay algo q depurar"), pidiendo revisión. **Ningún bug de U Core** — el log confirma arranque correcto (`⚡️ WebView loaded`) y todo lo demás es ruido normal de iOS/WebKit/Xcode (aviso de `UIScene` lifecycle pendiente de adoptar, sandbox extension al instalar en depuración, logs internos de WebKit/XPC sin relación con el código de la app). Documentado aquí solo el punto con sustancia real, que surgió de la propia conversación y Pablo pidió aparcar para revisarlo más adelante — **no se ha tocado ningún código**.

**Lo que sí es relevante, sin tocar nada:** `capacitor.config.ts` tiene `server.url: 'https://u-scout-production.up.railway.app'` — la app de iOS no lleva la web empaquetada dentro del binario, la carga en vivo desde el servidor cada vez que abre. Confirmado en el log (`⚡️ Loading app at https://u-scout-production.up.railway.app...`). Esto es una decisión ya tomada en el proyecto (no de esta sesión), con una implicación real: si el servidor no responde, la app no abre.

**a) Alternativas de hosting a Railway** (por si el motivo fuera cambiar de proveedor, no la arquitectura): Render (reemplazo más directo, mismo modelo deploy-por-git), Fly.io (más control, útil si en algún momento interesa desplegar más cerca de Asia/China por la base de usuarias WCBA), Heroku (clásico, ya sin capa gratuita, más caro hoy que Railway/Render para lo mismo), DigitalOcean App Platform, y AWS/GCP (mucho más control y escala, pero demasiada complejidad de gestión para el tamaño actual del proyecto). Ninguna de estas resuelve por sí sola el punto (b) de abajo — es un problema de arquitectura, no de proveedor.

**b) Empaquetar la web dentro del `.app` de iOS en vez de cargarla remota** (quitar `server.url`, usar el `webDir` local de Capacitor, `dist/public`, ya generado por `npm run build`):
- **Pro real**: la app abriría aunque el servidor de turno (Railway o el que sea) esté caído — hoy no abre en absoluto en ese caso.
- **Pero el beneficio es parcial, no total**: la app seguiría necesitando la API para todo lo que de verdad importa (login, datos, motor) — pasarías de "la app no abre" a "la app abre pero no puede hacer nada útil sin servidor". No hay ninguna capa de funcionalidad offline hoy.
- **Contra principal, y es grande en esta fase**: se pierde el despliegue instantáneo. Hoy, cualquier cambio (como los ~8 de esta sola sesión) está en producción, visible para todo el mundo, en menos de un minuto tras el push a Railway. Empaquetado local, cada cambio —por pequeño que sea— exigiría recompilar en Xcode, volver a firmar, subir build nueva a App Store Connect, pasar revisión de Apple (horas/días), y que cada usuaria/entrenador actualice la app en su móvil (muchos tardan semanas o nunca lo hacen) — con el riesgo añadido de versiones de la web desincronizadas entre usuarios distintos, cosa que hoy no puede pasar.
- Contras menores: `.ipa` algo más grande, más pasos de release que recordar.
- Vía intermedia sin explorar todavía, mencionada pero no investigada a fondo: plugins de "live update" (p. ej. Capgo, Ionic Appflow) permiten empaquetar local por defecto pero seguir empujando actualizaciones del JS/web sin pasar por revisión de Apple (solo cambios nativos reales necesitarían el ciclo completo) — candidato natural para cuando el ritmo de cambios baje y el argumento de resiliencia pese más que el de velocidad de iteración.

**Recomendación dada a Pablo, no ejecutada**: no empaquetar todavía, mientras el ritmo de iteración siga siendo tan alto como en esta sesión — el coste de perder despliegues instantáneos supera un beneficio que además es parcial. Revisar esta decisión más adelante, cuando la app esté más asentada y cambie con menos frecuencia (o si el "live update" de arriba se investiga y cambia el cálculo).
