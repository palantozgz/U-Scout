 ...
 *
 * D=25: cuernos recortados en y=427, conector escalado (scaleY=102/77≈1.32468) anclado en y=544.
 */
import { cn } from "@/lib/utils";

// Parte A — cuernos (izquierdo + derecho)
const U_HORNS_D =
...
export function UCoreLogoSvgLockup({
  ...
  const markSize = size;
  return (
    <span className={cn("inline-flex flex-col items-center gap-1", className)} aria-label="U Core" role="img">
      <svg
        width={markSize}
        height={markSize}
        viewBox="260 290 510 270"
        fill="none"
      ...
      >
        <defs>
          <clipPath id="ucore-lockup-horn-clip">
            <rect x={0} y={0} width={1024} height={HORN_CLIP_Y} />
          </clipPath>
        </defs>
        <g clipPath="url(#ucore-lockup-horn-clip)">
          <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d={U_HORNS_D} />
        </g>
        <g transform={CONN_SCALE}>
          <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d={U_CONN_D} />
        </g>
      </svg>
      <span
        className="text-foreground font-black tracking-[0.35em] uppercase select-none"
        style={{ fontSize: Math.round(markSize * 0.18) }}
      >
        CORE
      </span>
    </span>
    ...
    >
      <style>
        {`
          ...
          .ucore-boot-fadein { animation: ucore-boot-fadein 220ms ease-out both; }
        `}
      </style>
      <div
        className="ucore-boot-fadein text-foreground flex flex-col items-center"
        style={{ gap: "0.5rem" }}
      >
        <svg
        ...
        >
          <defs>
            <clipPath id="ucore-splash-horn-clip">
              <rect x={0} y={0} width={1024} height={HORN_CLIP_Y} />
            </clipPath>
          </defs>
          <g clipPath="url(#ucore-splash-horn-clip)">
            <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d={U_HORNS_D} />
          </g>
          <g transform={CONN_SCALE}>
            <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d={U_CONN_D} />
          </g>
          <path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d={U_SCOUT_D} />
        </svg>
        <span
          style={{
            fontSize: "11px",
...
// AuthLoadingSplash / legacy lockups removed (unused by current shell).
